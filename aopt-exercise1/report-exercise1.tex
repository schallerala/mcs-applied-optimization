...
\begin{enumerate}
    ...
    No, cf \simpleref{eq:eq1}, if we don't have the $a_i \ge 0$ for $\forall i$ constraint, it removes the constraint on the $\beta \in [0,1]$, which then means we could have $(1-\beta) < 0$. Therefore the step $(1-\beta)Ax_0 \preceq b$ is not doable, we might have $(1-\beta)Ax_0 \succeq b$. \hfill $\blacksquare$
\end{enumerate}
