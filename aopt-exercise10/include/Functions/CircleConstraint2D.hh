...
namespace AOPT {
    ...
    public:
        ...
        // constructor
        CircleConstraint2D(const int _n, const int _idx, const double _center_x, const double _center_y,
        ...
        // gradient evaluation
        // _x stores the coordinates of all nodes
        // _g stores the gradient of all nodes (make sure to only modify the correct entries)
        inline virtual void eval_gradient(const Vec &_x, Vec &_g) override {
            _g.setZero();
            //------------------------------------------------------//
            // implement the gradient and store in _g

            _g[2 * idx_]     = 2 * _x[2 * idx_]     - 2 * center_x_;
            _g[2 * idx_ + 1] = 2 * _x[2 * idx_ + 1] - 2 * center_y_;
